...
LocalStation::LocalStation(string name, int type, int distance, const ReadFile &read_file) : name_{name}, type_{type}, distance_{distance}, read_file_{read_file},
																							 trains_in_station_{}, tracks_state_{0, 0, 0, 0}, trains_ahead_east_{}, trains_ahead_weast_{}, parked_trains_east_{}, parked_trains_weast_{}
{
}
...
void LocalStation::ArrivalRequest(Train *t)
{
	...
	// da piu di 25 minuti s� � cosi allora metto il treno velocita in parcheggio se no aumento il
	// tempo di attesa del treno in stazione del tempo che il treno che sta per passare ci impiegher�
	// a superarli di 10km
	if (t->getType() > 1)
	{
		int time_to_clear = GetHighSpeedTrainTime(t);
		// controllo se nel tempo che ci impieaga a percorrere i 30km (20 + 5)
		// c'� un altro treno che deve partire in meno del tempo che ci impiegha il treno
		// a fare i 25km
		for (int i = t->getVerse(); i < 4; i += 2)
		{
			/////////////////////////
			bool max_wait_limit = (parked_trains.size() > 0) ? FindTrain(i, parked_trains)->GetWaitTime() >= FindTrain(i, parked_trains)->getMaxWait() : false;
			if (GetHighSpeedTrainTime(t) < tracks_state_[i] && max_wait_limit)
			{
				//?????if (trains.size() > 0 && trains[0]->getType() >= t->getType())
				AddParkedTrain(t);
			}
		}
	...
	*/
	...
	if (t->getState() == "p")
	{
		if (t->getType() >= 1)
		{
			...
			for (int i = t->getVerse(); i < 4 && !empty_park; i += 2)
		...
		if (tracks_state_[t->GetTrack()] == 0)
		{
			...
			parked_trains.erase(parked_trains.begin());
			// Controlla se � stazione origine o capolinea
